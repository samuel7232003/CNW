<%--
  Created by IntelliJ IDEA.
  User: Le Viet Thanh
  Date: 12/15/2023
  Time: 1:00 AM
  To change this template use File | Settings | File Templates.
--%>
<%@ page contentType="text/html;charset=UTF-8" language="java" %>
<html lang="en">
<head>
    <title>Viết về Hội An</title>
    <link rel="stylesheet" href="../template/poster.css">
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=Be+Vietnam+Pro:wght@300;400;700&family=Bodoni+Moda:ital,wght@0,400;0,500;0,600;0,700;0,800;1,400;1,500;1,600;1,700;1,800&family=Poppins:ital,wght@0,100;0,200;0,300;0,400;0,500;0,600;0,700;1,100;1,200;1,300;1,400;1,500;1,600;1,700&display=swap" rel="stylesheet">
</head>
<body>
<div class="header">
    <div>
        <a href="./home.html">Trang chủ</a>
        <a href="./poster.html" class="bold-underline">Bài viết</a>
        <a>Thông tin vé</a>
    </div>
    <a class="login">Đăng nhập</a>
</div>
<div class="main">
    <div class="title-main">
        <p>Viết về...</p>
        <img class="img" src="../image/icon.png">
        <span></span>
    </div>
    <div class="list-posts">
        <div class="colunm">
            <div class="item">
                <img src="../image/image1.jpg" alt="">
                <div class="text">
                    <p class="title">Dạo quanh phố cổ... </p>
                    <p class="content">với những cảm xúc như trở về một thời hoàng kim sống động, rực rỡ đầy màu sắc. Bạn sẽ như được chạm vào một lát cắt lịch sử, hiện hữu, chân thực, và đong đầy xúc cảm với những cảm xúc như trở về một thời hoàng kim sống động, rực rỡ đầy màu sắc. Bạn sẽ như được chạm vào một lát cắt lịch sử, hiện hữu, chân thực, và đong đầy xúc cảm...</p>
                    <p class="seemore">Xem thêm</p>
                </div>
            </div>
            <div class="item">
                <img src="../image/image2.jpg" alt="">
                <div class="text">
                    <p class="title">Dạo quanh phố cổ... </p>
                    <p class="content">với những cảm xúc như trở về một thời hoàng kim sống động, rực rỡ đầy màu sắc. Bạn sẽ như được chạm vào một lát cắt lịch sử, hiện hữu, chân thực, và đong đầy xúc cảm...</p>
                    <p class="seemore">Xem thêm</p>
                </div>
            </div>
        </div>
        <div class="colunm">
            <div class="item">
                <img src="../image/image3.jpg" alt="">
                <div class="text">
                    <p class="title">Dạo quanh phố cổ... </p>
                    <p class="content">với những cảm xúc như trở về một thời hoàng kim sống động, rực rỡ đầy màu sắc. Bạn sẽ như được chạm vào một lát cắt lịch sử, hiện hữu, chân thực, và đong đầy xúc cảm...Bạn sẽ như được chạm vào một lát cắt lịch sử, hiện hữu, chân thực, và đong đầy xúc cảm...</p>
                    <p class="seemore">Xem thêm</p>
                </div>
            </div>
            <div class="item">
                <img src="../image/image4.jpg" alt="">
                <div class="text">
                    <p class="title">Dạo quanh phố cổ... </p>
                    <p class="content">với những cảm xúc như trở về một thời hoàng kim sống động, rực rỡ đầy màu sắc. Bạn sẽ như được chạm vào một lát cắt lịch sử, hiện hữu, chân thực, và đong đầy xúc cảm...Bạn sẽ như được chạm vào một lát cắt lịch sử, hiện hữu, chân thực, và đong đầy xúc cảm...</p>
                    <p class="seemore">Xem thêm</p>
                </div>
            </div>
        </div>
        <div class="colunm">
            <div class="item">
                <img src="../image/image5.jpg" alt="">
                <div class="text">
                    <p class="title">Dạo quanh phố cổ... </p>
                    <p class="content">với những cảm xúc như trở về một thời hoàng kim sống động, rực rỡ đầy màu sắc. Bạn sẽ như được chạm vào một lát cắt lịch sử, hiện hữu, chân thực, và đong đầy xúc cảm với những cảm xúc như trở về một thời hoàng kim sống động, rực rỡ đầy màu sắc. Bạn sẽ như được chạm vào một lát cắt lịch sử, hiện hữu, chân thực, và đong đầy xúc cảm...</p>
                    <p class="seemore">Xem thêm</p>
                </div>
            </div>
            <div class="item">
                <img src="../image/hoian1.jpg" alt="">
                <div class="text">
                    <p class="title">Dạo quanh phố cổ... </p>
                    <p class="content">với những cảm xúc như trở về một thời hoàng kim sống động, rực rỡ đầy màu sắc. Bạn sẽ như được chạm vào một lát cắt lịch sử, hiện hữu, chân thực, và đong đầy xúc cảm với những cảm xúc như trở về một thời hoàng kim sống động, rực rỡ đầy màu sắc. Bạn sẽ như được chạm vào một lát cắt lịch sử, hiện hữu, chân thực, và đong đầy xúc cảm...</p>
                    <p class="seemore">Xem thêm</p>
                </div>
            </div>
        </div>
    </div>
</div>
<div class="footer">
    <p class="title">Thông tin liên hệ:</p>
    <span></span>
    <p>Tel: 0232323232</p>
    <p>Email: hoiantrip@gmail.com</p>
</div>
</body>
</html>
